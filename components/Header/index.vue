<template>
    <div class="bg-gray-200 w-full h-15 flex justify-between lg:justify-end items-center px-5">
        ...
        <img src="/image/qq.png" alt="Menu" class="w-8 h-8 block lg:hidden cursor-pointer" @click="toggleMenu"
            :aria-expanded="showMenu ? 'true' : 'false'" aria-controls="mobile-drawer" aria-label="Toggle menu" />
        ...
        <div id="mobile-drawer"
            class="fixed top-0 left-0 h-full w-80 bg-black text-white z-50 transition-transform duration-500 will-change-transform"
                    ...
                    <li v-for="item in items" :key="item.key">
                        ...
                        <template v-if="item.children?.length">
                            <button class="w-full flex items-center justify-between px-3 py-2 rounded hover:bg-gray-100"
                                ...
                                <svg class="w-4 h-4 transition-transform"
                                    :class="openSections.has(item.key) ? 'rotate-180' : ''" viewBox="0 0 24 24"
                                    fill="currentColor">
                            ...
                            <transition name="fade">
                                <ul v-if="openSections.has(item.key)" :id="`submenu-${item.key}`"
                                    ...
                                    <li v-for="child in item.children" :key="child.key">
                                        <NuxtLink :to="child.href" class="block px-2 py-2 rounded hover:bg-gray-100"
                                            @click="closeMenu">
                                            {{ child.label }}
                                        </NuxtLink>
                                    </li>
                        ...
                        <template v-else>
                            <NuxtLink :to="item.href"
                                class="flex items-center gap-2 px-3 py-2 rounded hover:bg-gray-100" @click="closeMenu">
                                <img :src="item.icon" :alt="item.label" class="w-5 h-5" />
                                <span class="font-medium">{{ item.label }}</span>
                            </NuxtLink>
                        </template>
