<template>
    <div class="container lg:mx-auto px-2 py-5 lg:px-5 lg:py-5">
        <h1 class="text-[40px]">Dashboard</h1>
        ...
        <div class="flex flex-col lg:flex-row lg:justify-between gap-3 lg:items-center">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <div class="relative z-10">
                    <!-- ปุ่ม Filter -->
                    <button @click="filterOpen = !filterOpen" :aria-expanded="filterOpen ? 'true' : 'false'"
                        aria-haspopup="menu" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl
           ...
           transition">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                            <path
                                d="M3.5 5a1.5 1.5 0 0 1 1.2-.6h14.6a1.5 1.5 0 0 1 1.2 2.4l-6.3 7.9v3.6a1.5 1.5 0 0 1-2.2 1.3l-3-1.7a1.5 1.5 0 0 1-.8-1.3v-1.9L3.5 6.8A1.5 1.5 0 0 1 3.5 5z" />
                        </svg>
                    ...
                    <!-- เมนูตัวเลือก -->
                    <div v-if="filterOpen" role="menu" class="absolute lg:right-0 mt-2 w-44 z-50
           bg-white rounded-xl shadow-xl ring-1 ring-black/5 py-2">
                        <button v-for="opt in filterOptions" :key="opt" role="menuitem"
                ...
                <div>
                    <template v-for="m in members" :key="m.id">
                        ...
                        <div class="grid grid-cols-[48px_1fr_1fr_1fr_1fr_1fr] items-center border-b last:border-b-0">
                            <!-- ปุ่มขยาย -->
                            <button class="h-14 flex items-center justify-center hover:bg-gray-50 transition"
                                @click="toggle(m.id)" :aria-expanded="expanded.has(m.id) ? 'true' : 'false'"
                                :aria-controls="`row-details-${m.id}`">
                                <svg class="w-5 h-5 transition-transform" :class="expanded.has(m.id) ? 'rotate-90' : ''"
                        ...
                        <!-- แถวรายละเอียด (ขยาย) -->
                        <div v-if="expanded.has(m.id)" :id="`row-details-${m.id}`"
...
const members = ref<Member[]>([
    { id: 1, memberId: '001', name: 'A', surname: 'AA', total: 1560, paid: 520 },
    { id: 2, memberId: '002', name: 'B', surname: 'BB', total: 1040, paid: null },
])
